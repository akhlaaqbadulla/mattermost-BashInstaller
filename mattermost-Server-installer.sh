...
# ---- Database Settings ---- #
echo "Please enter the database engine. Set to either mariadb or mysql"
read $dbselect
db_engine="$dbselect" # Set to "mariadb" or "mysql"
...
# ---- Edition Selection ---- #
echo "Please enter the Mattermost edition you want to install either enterprise or team."
read $edition
mm_edition="$edition" # Set to "enterprise" to download and install Enterprise edition.
...
if [[ "$db_engine" != "mariadb" && "$db_engine" != "mysql" ]]; then
  echo "ERROR - db_engine can only be set to mariadb or mysql - please enter again"
  read $dbselect
  exit 1
fi
